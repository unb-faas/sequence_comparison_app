...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    event = req.get_json()
    required_params = ["s1","s2","t1","t2","service","concurrence","repetition","execid", "container", "storage_account_string_connection"]
    errors = []
    ...
    if (len(errors) > 0):
        ...
        try:
            ...
            return func.HttpResponse(
                json.dumps({"error":"Failed connecting to Blob Storage - Check your connection string"}),
                status_code=500
        )
            _
        _s1 = event["s1"]
